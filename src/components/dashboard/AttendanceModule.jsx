...
// CORRECTED: This function has been updated to handle non-string inputs gracefully.
// The previous version would throw a TypeError if `value` was not a string
// (e.g., if it was a number or boolean). This update ensures that `value` is
// always converted to a string before being passed to `escapeHtml`.
...
const Button = React.memo(
  React.forwardRef(
    (
      ...
      const baseClasses =
        "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:h-4 [&_svg]:w-4";
...
const Badge = React.memo(
  ({ className, variant = "default", children, ...props }) => {
    ...
    const baseClasses =
      "inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-blue-700 focus:ring-offset-2";
...
const Input = React.memo(
  React.forwardRef(({ className, type = "text", ...props }, ref) => {
    const sanitizedProps = { ...props
    };
    if (sanitizedProps.value) {
      sanitizedProps.value = sanitizeInput(sanitizedProps.value);
    }
    return (
      <input
        ref={ref}
...
const Select = React.memo(
  ({ children, value, onValueChange, placeholder = "Select...", ...props }) => {
    ...
    const handleValueChangeInternal = (newValue) => {
      if (onValueChange) onValueChange(sanitizeInput(newValue));
      setOpen(false);
    };
    ...
    const displayedText = selectedChild
    ...
    return (
      <div className="relative" ref={selectRef} {...props}>
        <button
          ...
          aria-expanded={open}
        >
          <span>{sanitizeInput(displayedText)}</span>
          <ChevronDown
...
const SelectItem = React.memo(
  ({ children, value, onClick, className, selected, ...props }) => {
    return (
      <div
        className={className}
        onClick={onClick}
...
const LeaveActionModal = React.memo(
  ({ isOpen, onClose, leaveRequest, actionType, onConfirm }) => {
    ...
    const sanitizedStudentName = sanitizeInput(leaveRequest.studentName);
    const sanitizedStudentId = sanitizeInput(leaveRequest.studentId);
    const sanitizedReason = sanitizeInput(leaveRequest.reason);

    const handleConfirm = () => {
      onConfirm(leaveRequest.id, actionType, sanitizeInput(comment));
    };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4">
            ...
            <p className="text-sm">
              Dates: {new Date(leaveRequest.fromDate).toLocaleDateString()} -{" "}
              {new Date(leaveRequest.toDate).toLocaleDateString()}
            </p>
            <div>
              <label
                htmlFor="actionComment"
...
const LeaveDetailsModal = React.memo(({ isOpen, onClose, leaveRequest }) => {
  ...
  const sanitizedStudentName = sanitizeInput(leaveRequest.studentName);
  const sanitizedStudentId = sanitizeInput(leaveRequest.studentId);
  const sanitizedCourse = sanitizeInput(leaveRequest.course);
  const sanitizedSemester = sanitizeInput(leaveRequest.semester);
  const sanitizedLeaveType = sanitizeInput(leaveRequest.leaveType);
  const sanitizedReason = sanitizeInput(leaveRequest.reason);
  const sanitizedComments = sanitizeInput(leaveRequest.comments);
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Badge className={getLeaveStatusColor(leaveRequest.status)}>
              {sanitizeInput(leaveRequest.status)}
            </Badge>
          </div>
          ...
          {leaveRequest.attachment && (
            <div className="flex items-center text-blue-600">
              ...
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  console.log("Simulating attachment download.");
                }}
              >
                View Attachment
              </a>
...
const TabsList = React.memo(
  ({ className, children, activeTab, setActiveTab, ...props }) => (
    <div
...
const TabsTrigger = React.memo(
  ({ value, className, children, active, onClick, ...props }) => (
    <button
...
const AttendanceModule = () => {
  ...
  const [classSummaryDate, setClassSummaryDate] = useState(
  ...
  const [currentLeaveAction, setCurrentLeaveAction] = useState({
    leaveId: null,
    action: "",
    comment: "",
  });
  ...
  const handleTopicDiscussedChange = useCallback(
    (e) => setTopicDiscussed(sanitizeInput(e.target.value)),
    []
  );
  const handleRemarksChange = useCallback(
    (e) => setRemarks(sanitizeInput(e.target.value)),
    []
  );
  const handleClassSummaryCourseChange = useCallback(
    (value) => setClassSummaryCourse(sanitizeInput(value)),
    []
  );
  const handleClassSummaryDateChange = useCallback(
    (e) => setClassSummaryDate(sanitizeInput(e.target.value)),
    []
  );
  const handleTopicCoveredChange = useCallback(
    (e) => setTopicCovered(sanitizeInput(e.target.value)),
    []
  );
  const handleDurationChange = useCallback(
    (e) => setDuration(sanitizeInput(e.target.value)),
    []
  );
  const handleDetailedNotesChange = useCallback(
    (e) => setDetailedNotes(sanitizeInput(e.target.value)),
    []
  );
  const handleHomeworkChange = useCallback(
    (e) => setHomework(sanitizeInput(e.target.value)),
    []
  );
  const handleNextClassPrepChange = useCallback(
    (e) => setNextClassPrep(sanitizeInput(e.target.value)),
    []
  );
  const handleLeaveFilterCourseChange = useCallback(
    (value) => setLeaveFilter((prev) => ({
      ...prev,
      course: sanitizeInput(value),
    })),
    []
  );
  const handleLeaveFilterSemesterChange = useCallback(
    (value) => setLeaveFilter((prev) => ({
      ...prev,
      semester: sanitizeInput(value),
    })),
    []
  );
  const handleLeaveFilterStatusChange = useCallback(
    (value) => setLeaveFilter((prev) => ({
      ...prev,
  ...
  const handleLeaveAction = useCallback((leaveId, action, comment) => {
    ...
    setStudentLeaveRequests((prev) =>
      prev.map((leave) =>
        ...
        {
          ...leave,
          status: sanitizedAction,
          comments: sanitizedComment || leave.comments,
        } :
        leave
      )
    );
    setAlertModalContent({
      ...
      message: `Leave request ${leaveId} ${sanitizedAction}ed.`,
    });
    setShowAlertModal(true);
    setShowLeaveActionModal(false);
    setCurrentLeaveAction({
      leaveId: null,
      action: "",
      comment: "",
    });
  ...
  const exportLeaveData = useCallback((format) => {
    ...
    setAlertModalContent({
      title: "Export Successful",
      message: `Leave requests exported as ${format.toUpperCase()}.`,
    });
    setShowAlertModal(true);
  }, []);
  ...
  const saveAttendance = useCallback(() => {
    ...
    const sanitizedTopicDiscussed = sanitizeInput(topicDiscussed);
    const sanitizedRemarks = sanitizeInput(remarks);

    console.log("Saving attendance for course:", selectedCourse);
    console.log("Topic Discussed:", sanitizedTopicDiscussed);
    console.log("Remarks:", sanitizedRemarks);
    console.log(
      "Student Attendance Records:",
      currentAttendance.map((s) => ({
        id: s.id,
        rollNo: s.rollNo,
        name: s.name,
        status: s.status,
    ...
    setAlertModalContent({
      ...
      message: `Attendance and class details saved for ${
        courses.find((c) => c.code === selectedCourse)?.name
      }.`,
    });
    setShowAlertModal(true);
  ...
  const handleSaveClassSummaryDraft = useCallback(() => {
    const classSummary = {
      course: sanitizeInput(classSummaryCourse),
      date: sanitizeInput(classSummaryDate),
      topicCovered: sanitizeInput(topicCovered),
      duration: sanitizeInput(duration),
      detailedNotes: sanitizeInput(detailedNotes),
      homework: sanitizeInput(homework),
      nextClassPrep: sanitizeInput(nextClassPrep),
      status: "draft",
    };
    console.log("Saving Class Summary Draft:", classSummary);
    setAlertModalContent({
      title: "Class Summary Saved",
      message: "Class Summary saved as draft!",
    });
    setShowAlertModal(true);
  }, [
    classSummaryCourse,
  ...
  const handlePublishClassSummary = useCallback(() => {
    ...
    setAlertModalContent({
      title: "Class Summary Published",
      message: "Class Summary published successfully!",
    });
    setShowAlertModal(true);
  }, [
    classSummaryCourse,
  ...
  const filteredLeaveRequests = studentLeaveRequests.filter((leave) => {
    return (
      (leaveFilter.course === "all" ||
        sanitizeInput(leave.course) === sanitizeInput(leaveFilter.course)) &&
      (leaveFilter.semester === "all" ||
        sanitizeInput(leave.semester) === sanitizeInput(leaveFilter.semester)) &&
      (leaveFilter.status === "all" ||
        sanitizeInput(leave.status) === sanitizeInput(leaveFilter.status))
    );
  });
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <Tabs defaultValue="mark-attendance" className="space-y-6">
        <div className="sm:hidden bg-white relative">
          ...
          >
            ...
            {mobileMenuOpen ? (
              <ChevronUp className="h-4 bg-white w-4" />
            ) : (
            ...
            <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg">
              <div className="flex bg-white flex-col p-2 space-y-1 w-full">
                ...
                <TabsTrigger
                  ...
                  active={activeTab === "student-leave"}
                >
        ...
        <TabsList
          className="hidden sm:grid w-full grid-cols-4 gap-2 bg-gray-50 p-1 rounded-lg"
          activeTab={activeTab}
        ...
        <TabsContent
          value="mark-attendance"
          activeTab={activeTab}
          className="pt-4 sm:pt-6 space-y-6"
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {courses.map((course) => (
              <Card
                ...
                onClick={() => setSelectedCourse(sanitizeInput(course.code))}
              >
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <CardTitle className="text-lg sm:text-xl">
                        {sanitizeInput(course.name)}
                      </CardTitle>
                      <Badge variant="outline" className="mt-2">
                        {sanitizeInput(course.code)}
                      </Badge>
                    </div>
                    <div className="flex flex-col items-end space-y-1">
                      <div className="flex items-center text-sm text-gray-600">
                        <Clock className="h-4 w-4 mr-1" />
                        <span>{sanitizeInput(course.time)}</span>
                      </div>
                      <div className="flex items-center text-sm text-gray-600">
                        <Users className="h-4 w-4 mr-1" />
                        <span>{sanitizeInput(course.students)} students</span>
                      </div>
                    </div>
                  </div>
                </CardHeader>
            ...
            <CardContent>
              <div className="space-y-3">
                {attendanceData[selectedCourse]?.map((student) => (
                  <div
                    key={student.id}
                    className="p-4 rounded-lg shadow-2xs transition-colors"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-center space-x-4">
                        <div
                          className={`w-3 h-3 rounded-full ${getDisciplineColor(
                            student.disciplineCode
                          )}`}
                        ></div>
                        <div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                        <div className="flex gap-4">
                          ...
                          <div className="text-center">
                            ...
                            >
                              {sanitizeInput(student.attendancePercent)}%
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge className={getStatusColor(student.status)}>
                            {sanitizeInput(student.status)}
                          </Badge>
        ...
        >
          <Card className="border border-gray-200">
            <CardHeader>
              ...
              <CardDescription>
                View detailed attendance statistics and reports
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="class-summary" activeTab={activeTab} className="pt-4 sm:pt-6">
          <Card className="border border-gray-200">
            <CardHeader>
              ...
              <CardDescription>
                Add comprehensive notes and summary for today's class
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={handleSaveClassSummaryDraft}>
                  Save Draft
                </Button>
                <Button onClick={handlePublishClassSummary}>
                  Publish Summary
                </Button>
              </div>
            </CardContent>
                        ...
                        <TableRow key={leave.id}>
                          <TableCell className="font-medium">
                            <div className="flex flex-col">
                              <span>
                                {sanitizeInput(leave.studentName)}
                              </span>
                              <span className="text-sm text-gray-600">
                                {sanitizeInput(leave.studentId)}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell>
                            <div className="flex flex-col">
                              <span>{sanitizeInput(leave.course)}</span>
                              <span className="text-sm text-gray-600">
                                {sanitizeInput(leave.semester)}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell>
                    ...
                    <CardTitle className="text-3xl">
                      {
                        studentLeaveRequests.filter(
                          (l) =>
                            l.status === "approved" &&
                            new Date(l.appliedDate).getMonth() ===
                              new Date().getMonth()
                        ).length
                      }
                    </CardTitle>
      ...
      <LeaveActionModal
        ...
        leaveRequest={studentLeaveRequests.find(
          (req) => req.id === currentLeaveAction.leaveId
        )}
